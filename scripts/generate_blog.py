...
def make_blog_page(d, t):
    ...
    faq_schema = json.dumps({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":x["q"],"acceptedAnswer":{"@type":"Answer","text":x["a"]}} for x in d.get("faq",[])]})
    art_schema = json.dumps({"@context":"https://schema.org","@type":"Article","headline":d["title"],"datePublished":iso,"author":{"@type":"Organization","name":"MoreBuildtech Expert Team"},"publisher":{"@type":"Organization","name":"MoreBuildtech","url":SITE_URL},"description":d["meta"],"url":url})
    bc_schema  = json.dumps({"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"Home","item":SITE_URL},{"@type":"ListItem","position":2,"name":"Blog","item":f"{SITE_URL}/blog"},{"@type":"ListItem","position":3,"name":d["title"],"item":url}]})
...
<meta property="og:type" content="article"/><meta property="og:title" content="{d['title']}"/>
<meta property="og:description" content="{d['meta']}"/><meta property="og:url" content="{url}"/>
...
*{{margin:0;padding:0;box-sizing:border-box;}}body{{background:var(--black);color:var(--white);font-family:'Montserrat',sans-serif;font-weight:300;overflow-x:hidden;}}
...
.logo{{font-family:'Cormorant Garamond',serif;font-size:1.2rem;font-weight:600;letter-spacing:4px;color:var(--white);text-transform:uppercase;text-decoration:none;}}.logo span{{color:var(--gold);}}
.nav-links{{display:flex;gap:32px;list-style:none;}}.nav-links a{{font-size:0.65rem;letter-spacing:3px;text-transform:uppercase;color:rgba(248,244,238,0.6);text-decoration:none;transition:color 0.3s;}}.nav-links a:hover{{color:var(--gold);}}
.nav-cta{{border:1px solid var(--gold)!important;color:var(--gold)!important;padding:8px 22px;}}.nav-cta:hover{{background:var(--gold)!important;color:var(--black)!important;}}
...
.breadcrumb a{{color:var(--grey);text-decoration:none;}}.breadcrumb a:hover{{color:var(--gold);}}.breadcrumb span{{margin:0 8px;color:var(--gold);}}
...
.blog-meta{{display:flex;flex-wrap:wrap;gap:24px;font-size:0.65rem;letter-spacing:2px;text-transform:uppercase;color:var(--grey);}}.blog-meta span::before{{content:'— ';color:var(--gold);}}
...
article ul,article ol{{margin:12px 0 20px 24px;}}article li{{margin-bottom:10px;font-size:0.9rem;line-height:1.8;color:rgba(248,244,238,0.7);}}
article strong{{color:var(--white);font-weight:500;}}article a{{color:var(--gold);text-decoration:none;}}
.tags-row{{display:flex;flex-wrap:wrap;gap:8px;margin:40px 0;}}.b-tag{{background:transparent;border:1px solid rgba(201,168,76,0.25);color:var(--grey);font-size:0.6rem;letter-spacing:2px;text-transform:uppercase;padding:5px 14px;}}
...
.faq-item{{border-bottom:1px solid var(--glass-border);padding:20px 0;}}.faq-item:last-child{{border-bottom:none;}}
.faq-q{{font-size:0.85rem;font-weight:500;letter-spacing:1px;color:var(--gold);margin-bottom:10px;}}.faq-a{{font-size:0.88rem;line-height:1.85;color:rgba(248,244,238,0.65);}}
.cta-box{{background:linear-gradient(135deg,rgba(201,168,76,0.08),rgba(201,168,76,0.03));border:1px solid var(--glass-border);padding:48px;text-align:center;margin:48px 0;}}
...
.btn-gold{{display:inline-block;background:linear-gradient(135deg,var(--gold),var(--gold2));color:var(--black);padding:14px 40px;font-size:0.65rem;letter-spacing:4px;text-transform:uppercase;text-decoration:none;font-weight:600;}}
...
.footer-logo{{font-family:'Cormorant Garamond',serif;font-size:1rem;font-weight:600;letter-spacing:4px;text-transform:uppercase;color:var(--white);}}.footer-logo span{{color:var(--gold);}}
footer p{{font-size:0.65rem;color:var(--grey);}}footer a{{color:var(--gold);text-decoration:none;}}
.wa-float{{position:fixed;bottom:32px;right:32px;z-index:9999;}}.wa-btn{{width:56px;height:56px;background:linear-gradient(135deg,#25D366,#128C7E);border-radius:50%;display:flex;align-items:center;justify-content:center;text-decoration:none;box-shadow:0 6px 24px rgba(37,211,102,0.4);}}.wa-btn svg{{width:26px;height:26px;fill:#fff;}}
@media(max-width:768px){{nav{{padding:16px 20px;}}.blog-hero{{padding:40px 20px 32px;}}footer{{padding:24px;flex-direction:column;text-align:center;}}}}
</style>
</head>
<body>
<nav>
...
<div class="breadcrumb"><a href="{SITE_URL}">Home</a><span>›</span><a href="{SITE_URL}/blog">Blog</a><span>›</span>{t['cat']}</div>
<div class="blog-hero">
  <div class="b-cat">{t['cat']}</div>
  <h1>{d['title']}</h1>
  <div class="blog-meta"><span>{date}</span><span>{d.get('reading_time','6 min read')}</span><span>MoreBuildtech Expert Team</span><span>Mumbai</span></div>
</div>
<div class="blog-body">
  {d['html']}
  <div class="tags-row">{tags}</div>
  <div class="faq-section"><h2>Frequently Asked Questions</h2>{faqs}</div>
  <div class="cta-box">
    <div class="cta-tag">Free Consultation</div>
    <h3>Ready to Build Your Dream Space?</h3>
    <p>Mumbai's trusted interior & construction experts — 7+ years, 50+ projects, 98% satisfaction.</p>
...
<div class="wa-float"><a class="wa-btn" href="{WA_LINK}" target="_blank"><svg viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a></div>
</body></html>"""

def make_index(posts):
    cards = ""
    for p in sorted(posts, key=lambda x: x["date_iso"], reverse=True)[:50]:
        cards += f'<article class="b-card"><div class="b-cat-badge">{p["cat"]}</div><h2 class="b-card-title"><a href="{SITE_URL}/blog/{p["slug"]}.html">{p["title"]}</a></h2><p class="b-card-excerpt">{p["excerpt"]}</p><div class="b-card-footer"><span>{p["date"]}</span><a href="{SITE_URL}/blog/{p["slug"]}.html">Read More →</a></div></article>'
...
<meta name="description" content="Expert interior design and construction tips for Indian homes by MoreBuildtech Mumbai."/>
<link rel="canonical" href="{SITE_URL}/blog"/>
<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400;600;700&family=Montserrat:wght@300;400;500;600&display=swap" rel="stylesheet"/>
<style>
:root{{--black:#080808;--deep:#0d0d0d;--panel:#111111;--gold:#c9a84c;--white:#f8f4ee;--grey:#888;--glass-border:rgba(201,168,76,0.2);}}
*{{margin:0;padding:0;box-sizing:border-box;}}body{{background:var(--black);color:var(--white);font-family:'Montserrat',sans-serif;font-weight:300;}}
...
.logo{{font-family:'Cormorant Garamond',serif;font-size:1.2rem;font-weight:600;letter-spacing:4px;color:var(--white);text-transform:uppercase;text-decoration:none;}}.logo span{{color:var(--gold);}}
.nav-links{{display:flex;gap:32px;list-style:none;}}.nav-links a{{font-size:0.65rem;letter-spacing:3px;text-transform:uppercase;color:rgba(248,244,238,0.6);text-decoration:none;}}.nav-links a:hover{{color:var(--gold);}}
.nav-cta{{border:1px solid var(--gold)!important;color:var(--gold)!important;padding:8px 22px;}}
.blog-hero{{background:var(--deep);padding:72px 60px;text-align:center;border-bottom:1px solid var(--glass-border);}}
.hero-tag{{font-size:0.6rem;letter-spacing:5px;text-transform:uppercase;color:var(--gold);margin-bottom:20px;}}
...
.blog-hero p{{font-size:0.85rem;color:var(--grey);}}
...
.b-card{{background:var(--black);padding:36px;transition:background 0.3s;}}.b-card:hover{{background:var(--panel);}}
...
.b-card-title{{font-family:'Cormorant Garamond',serif;font-size:1.35rem;font-weight:400;line-height:1.4;margin-bottom:12px;}}.b-card-title a{{color:var(--white);text-decoration:none;}}.b-card-title a:hover{{color:var(--gold);}}
.b-card-excerpt{{font-size:0.8rem;line-height:1.8;color:var(--grey);margin-bottom:20px;}}
.b-card-footer{{display:flex;justify-content:space-between;font-size:0.6rem;letter-spacing:2px;text-transform:uppercase;color:var(--grey);border-top:1px solid var(--glass-border);padding-top:16px;}}.b-card-footer a{{color:var(--gold);text-decoration:none;}}
footer{{background:var(--deep);border-top:1px solid var(--glass-border);padding:32px 60px;display:flex;justify-content:space-between;flex-wrap:wrap;gap:16px;margin-top:64px;}}
.footer-logo{{font-family:'Cormorant Garamond',serif;font-size:1rem;font-weight:600;letter-spacing:4px;text-transform:uppercase;}}.footer-logo span{{color:var(--gold);}}
footer p{{font-size:0.65rem;color:var(--grey);}}footer a{{color:var(--gold);text-decoration:none;}}
@media(max-width:768px){{nav{{padding:16px 20px;}}.blog-hero{{padding:48px 20px;}}.grid{{padding:0;margin:32px 0;}}.b-card{{padding:24px;}}footer{{padding:24px;flex-direction:column;}}}}
</style>
</head>
<body>
<nav><a class="logo" href="{SITE_URL}">MORE<span>BUILD</span>TECH</a><ul class="nav-links"><li><a href="{SITE_URL}/#services">Services</a></li><li><a href="{SITE_URL}/blog">Blog</a></li><li><a class="nav-cta" href="{WA_LINK}">Get Quote</a></li></ul></nav>
<div class="blog-hero"><div class="hero-tag">Expert Knowledge</div><h1>Interior & Construction Blog</h1><p>Design ideas, construction guides & expert tips for Indian homes — by MoreBuildtech, Mumbai</p></div>
<div class="grid">{cards}</div>
<footer><div class="footer-logo">MORE<span>BUILD</span>TECH</div><p>© {datetime.now().year} MoreBuildtech — Mumbai</p><p><a href="{SITE_URL}">Home</a> | <a href="{WA_LINK}">WhatsApp</a></p></footer>
</body></html>"""
...
def main():
    ...
    if not any(p["slug"] == slug for p in posts):
        posts.append({"slug":slug,"title":d["title"],"excerpt":d.get("excerpt",""),"cat":t["cat"],"date":datetime.now().strftime("%b %d, %Y"),"date_iso":datetime.now().strftime("%Y-%m-%d")})
        save_registry(posts)

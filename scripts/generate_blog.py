...
Same dark luxury design as morebuildtech.in
...
{{
  ...
  "html": "<article>...full content with h2,h3,p,ul,strong tags...</article>",
  "faq": [{{"q":"Q?","a":"Answer 2-3 sentences."}}]
}}"""
...
def make_blog_page(d, t):
    ...
    faqs  = "".join(f'<div class="faq-item"><div class="faq-q">{x["q"]}</div><div class="faq-a">{x["a"]}</div></div>' for x in d.get("faq",[]))
...
.b-cat{{display:inline-block;background:transparent;border:1px solid var(--gold);color:var(--gold);font-size:0.6rem;letter-spacing:3px;text-transform:uppercase;padding:5px 16px;margin-bottom:20px;}}
...
/* WA FLOAT */
.wa-float{{position:fixed;bottom:32px;right:32px;z-index:9999;display:flex;align-items:center;gap:12px;}}
...
.wa-btn:hover{{transform:scale(1.1);border-bottom:none;}}
...
<div class="wa-float">
  <a class="wa-btn" href="{WA_LINK}" target="_blank" aria-label="Chat on WhatsApp">
...
.nav-links{{display:flex;gap:32px;list-style:none;}}
.nav-links a{{font-size:0.65rem;letter-spacing:3px;text-transform:uppercase;color:rgba(248,244,238,0.6);text-decoration:none;transition:color 0.3s;}}
.nav-links a:hover,.nav-cta{{color:var(--gold);}}
.nav-cta{{border:1px solid var(--gold)!important;padding:8px 22px;}}
